...
local function getDisplayHeaderMenu()
    local headerMenu = {
        {
            header = Config.Label,
            txt = 'View LSPD Helicopters',
            params = {
                event = 'qb-helirental:client:openHeliMenuMenu'
            }
        }
    }
    return headerMenu
...
RegisterNetEvent('qb-helirental:client:openHeliMenuMenu', function()
    local ordered_keys = {}
    for k in pairs(Config.Helis) do
        table.insert(ordered_keys, k)
    end
    table.sort(ordered_keys)
    local heliMenu = {
        {
            header = "LSPD Vehicles",
            isMenuHeader = true
        }
    }
    for i = 1, #ordered_keys do
        local v = Config.Helis[ordered_keys[i]]
        if v.available then
            heliMenu[#heliMenu + 1] = {
                header = v.label,
                ...
                params = {
                    isServer = true,
                    event = 'qb-helirental:server:swapVehicle',
                    args = {
                        vehicle = v
                    }
                }
    ...
    exports['qb-menu']:openMenu(heliMenu)
end)

RegisterNetEvent('qb-helirental:client:swapVehicle', function(data)
    local heli = data.vehicle
...
RegisterNetEvent('qb-helirental:client:openHeliMenuRentalConfirmationMenu', function(data)
    local heli = data.vehicle
    local menu = {
        {
            header = 'Is this the helicopter you want to take out?',
            isMenuHeader = true
        },
        {
            header = heli.label .. '',
            txt = 'To Rent: $' .. heli.rental_cost .. ''
        },
        {
        ...
        },
        {
            header = 'Yes, please!',
            params = {
                event = 'qb-helirental:client:RentHeli',
                args = {
                    vehicle = heli
                }
            }
...
RegisterNetEvent('qb-helirental:client:RentHeli', function(data)
    local heli = data.vehicle
    local model = heli.model
    local cost = heli.rental_cost
    local spawn = Config.RentalLocations[Closest_Display].spawn_coords
    local x = spawn.x
    local y = spawn.y
    local z = spawn.z
    QBCore.Functions.TriggerCallback('qb-helirental:server:CanRentHeli', function(can)
        if can then
...
-- Display vehicles + zone & blip config
...
-- vehicles rental walk up config
Citizen.CreateThread(function()
    while true do
                ...
                if v.active then
                    ...
                    if dist < 12 then
                        ...
                        if not Config.WalkUpRentalMenu and dist < 2 then
                            DrawText3Ds(v.rental_coords.x, v.rental_coords.y, v.rental_coords.z + 0.9, '~g~E~w~ - View LSPD Helicopters')
                            if IsControlJustPressed(0, 38) then
                                setClosestDisplay()
                                TriggerEvent('qb-helirental:client:openHeliMenuMenu')
                            end
                        end
                        inRange = true
                    end
